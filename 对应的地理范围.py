...
class user_location(object):
    ...
    def get_wgs(self):
        ...
        url = 'http://apis.baidu.com/rtbasia/ip_location/ip_location?ip=%s&v=1.1' % (self.ip)
...
class location2sqlite(user_location):                                  #获取IP对应的GPS地址到数据库
    ...
    def list2sqlite(self):                                             #发送到数据库
        ...
        try:
            cursor.execute("insert into ip2location3306pure (num,id,ip,lat,lng,radius) values (?,?,?,?,?,?)",data)
        finally:
...
with open('E:\PycharmProjects\dbsample1\data\khh_ip_0033.201606.csv','r') as f1:
     ...
     for row in rawfile:
         ...
         if hefei.ip == 'None' or num < 14 or num > 100:
            continue
         hefei.list2sqlite()
         num = num + 1
         hefei.num = num
