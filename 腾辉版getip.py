...
class location2sqlite(user_location):                                  #获取IP对应的GPS地址到数据库
    ...
    def list2sqlite(self):                                             #发送到数据库
        ...
        try:
            cursor.execute("insert into ip2location25306 (num,id,ip,lat,lng,radius) values (?,?,?,?,?,?)",data)
        finally:
...
with open('E:\PycharmProjects\dbsample1\data\khh_ip_0253.201606.csv','r') as f1:
     num = 1
